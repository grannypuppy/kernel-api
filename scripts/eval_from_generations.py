...
class EvalConfig(Config):
    def __init__(self):
        ...
        # Clean up cache for the run after evaluation
...
def fetch_kernel_from_disk(run_dir: str, level: int, problem_id: int, sample_id: int, itr_id: int) -> str | None:
    """
    Fetch kernel file from disk (stored in runs/{run_name})
    """
    kernel_path = os.path.join(run_dir, f"level_{level}_problem_{problem_id}_sample_{sample_id}_itr__{itr_id}_kernel.py")
    new_kernel_path = os.path.join(run_dir, f"level_{level}_problem_{problem_id}_sample_{sample_id}_itr_{itr_id}_kernel.py")
    if os.path.exists(kernel_path):
        os.rename(kernel_path, new_kernel_path)
    if os.path.exists(new_kernel_path):
        kernel_path = new_kernel_path
    
    if os.path.exists(kernel_path):
        # Rename the kernel_path to another file name before reading
...
def evaluate_single_sample(work_args: WorkArgs, configs: EvalConfig, dataset, run_dir: str) -> KernelExecResult | None:
    ...
    kernel_src = fetch_kernel_from_disk(run_dir, configs.level, problem_id, sample_id, configs.itr_id)
            ...
            with mp.Pool(batch_size) as pool:
                ...
                for problem_id, sample_id, result in results:
                    ...
                    if result is not None:
                        ...
                        add_to_eval_results_file(problem_id, sample_id, result, eval_file_path, config)
...
def add_to_eval_results_file(problem_id: int, sample_id: int, eval_result: KernelExecResult, eval_file_path: str, config: EvalConfig):
    """
    ...
    eval_path = f'{run_dir}/level_{config.level}_problem_{problem_id}_sample_{sample_id}_itr_{config.itr_id}.json'
    eval_result_json = {
        'sample_id': sample_id,
        'itr_id': config.itr_id,
    ...
    with open(eval_path, 'w') as f:
        json.dump(eval_result_json, f)
    
    # Add new result
    if str(problem_id) not in eval_results:
        eval_results[str(problem_id)] = []
    eval_results[str(problem_id)].append(eval_result_json)
...
def main(config: EvalConfig):
    ...
    run_dir = os.path.join(config.runs_dir, config.run_name)
    eval_file_path = os.path.join(run_dir, f"eval_itr_{config.itr_id}_results.json")
    ...
    if config.cleanup_cache:
        run_cache_dir = os.path.join(config.kernel_eval_build_dir, config.run_name)
        if os.path.exists(run_cache_dir):
            print(f"\n[INFO] Cleaning up cache directory for run '{config.run_name}': {run_cache_dir}")
            try:
                shutil.rmtree(run_cache_dir)
                print(f"[INFO] Cache directory removed: {run_cache_dir}")
            except Exception as e:
                print(f"\n[WARNING] Failed to remove cache directory {run_cache_dir}: {str(e)}")
